...
export default function SendInvitesPage() {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"2rem 1.5rem", fontFamily:"Georgia, serif" }}>
        ...
        <div>
          ...
          {TEMPLATES.map(t => (
            <div key={t.id} onClick={() => setTemplate(t.id)}
              style={{ ...cardStyle, border: template===t.id ? "2px solid #2c1810" : "1px solid #e8ddd0", marginBottom:8, cursor:"pointer" }}>
        ...
        <div>
          ...
          {CHANNEL_OPTIONS.map(c => (
            <div key={c.value} onClick={() => setChannel(c.value)}
              style={{ ...cardStyle, border: channel===c.value ? "2px solid #2c1810" : "1px solid #e8ddd0", marginBottom:8, cursor:"pointer" }}>
      ...
      <div style={{ marginBottom:24 }}>
        ...
        {fetching ? (
          ...
          <div style={{ maxHeight:320, overflowY:"auto", border:"1px solid #e8ddd0", borderRadius:8 }}>
            {guests.length === 0 ? (
              <p style={{ padding:16, color:"#9e8878", fontSize:13 }}>No guests found.</p>
            ) : guests.map((guest, i) => (
      ...
      <button onClick={handleSend} disabled={loading || selectedGuests.length === 0}
        style={{
