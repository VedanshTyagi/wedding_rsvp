...
export default function SeatingPage() {
  ...
  const wedding_id = params?.weddingId
  ...
  async function fetchData() {
    ...
    const [{ data: tablesData }, { data: guestsData }, { data: assignmentsData }] =
      await Promise.all([
        supabase.from('seating_tables').select('*').eq('wedding_id', wedding_id).order('created_at'),
        supabase.from('guests').select('id, full_name, side, group_tag, dietary_preference').eq('wedding_id', wedding_id).order('full_name'),
    ...
    const tablesWithGuests = (tablesData || []).map(table => ({
      ...table,
      guests: assignments.filter(a => a.table_id === table.id).map(a => (guestsData || []).find(g => g.id === a.guest_id)).filter(Boolean),
    }))
  ...
  async function handleAddTable(e) {
    ...
    setAddingTable(true)
    await supabase.from('seating_tables').insert({ wedding_id, name: newTableName.trim(), capacity: Number(newTableCapacity) })
    setNewTableName('')
  ...
  async function handleAssign(guestId, tableId) {
    if (!tableId) return
    await supabase.from('seating_assignments').upsert({ guest_id: guestId, table_id: tableId, wedding_id }, { onConflict: 'guest_id,wedding_id' })
    fetchData()
  }
  ...
  if (loading) return <div className="text-sm text-muted-foreground py-12 text-center">Loading seating plan...</div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="border border-border rounded-xl p-4 space-y-3">
          <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wide">Unassigned ({unassigned.length})</h2>
          {unassigned.length === 0 ? (
        ...
        <div className="lg:col-span-2 space-y-4">
          <form onSubmit={handleAddTable} className="flex gap-2">
            <input value={newTableName} onChange={e => setNewTableName(e.target.value)} placeholder="Table name (e.g. Table 1, Family)"
            ...
            <input type="number" value={newTableCapacity} onChange={e => setNewTableCapacity(e.target.value)} min={1} max={30} placeholder="Seats"
                ...
                return (
                  <div key={table.id} className="border border-border rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => handleDeleteTable(table.id)} className="text-xs text-red-400 hover:text-red-600 transition-colors">Delete</button>
                    </div>
                    <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all ${occupancy >= 1 ? 'bg-red-400' : occupancy >= 0.8 ? 'bg-amber-400' : 'bg-green-400'}`}
                      ...
                      {table.guests.length === 0 ? (
                        ...
                        table.guests.map(guest => (
                          <div key={guest.id} className="flex items-center justify-between gap-2">
                            ...
                            <button onClick={() => handleUnassign(guest.id)} className="text-[10px] text-muted-foreground hover:text-red-500 transition-colors flex-shrink-0">Remove</button>
                          </div>
                        ))
                      )}
                    </div>
