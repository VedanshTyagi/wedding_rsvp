...
export default function Page() {
  return (
    <Suspense fallback={
      ...
      <GuestSelfCheckinPage />
    </Suspense>
  )
}

function GuestSelfCheckinPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [
        ...
        supabase.from('guests').select('id, full_name, group_tag').eq('id', guestId).single(),
        functionId ? supabase.from('wedding_functions').select('name').eq('id', functionId).single() : Promise.resolve({ data: null }),
        functionId ? supabase.from('checkin_log').select('id').eq('guest_id', guestId).eq('function_id', functionId).maybeSingle() : Promise.resolve({ data: null }),
      ])
  ...
  async function handleCheckIn() {
    if (!functionId) { setError('No function specified in QR code'); return }
    setChecking(true)
  ...
  if (checkedIn || alreadyCheckedIn) return (
    <div style={{ minHeight:'100vh', background:'#FAF6EF', display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem' }}>
      <div style={{ textAlign:'center', maxWidth:'340px' }}>
        <div style={{ width:'80px', height:'80px', borderRadius:'50%', background:'#BFA054', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 1.5rem', fontSize:'2.2rem', color:'#FAF6EF' }}>✓</div>
        ...
        <p style={{ fontFamily:'Georgia, serif', fontSize:'1.2rem', color:'#9A2143', marginBottom:'1rem' }}>{guest?.full_name}</p>
        {functionName && (
        ...
        <p style={{ color:'#6B6B6B', fontSize:'0.85rem', lineHeight:1.6 }}>
          {alreadyCheckedIn ? 'You have already checked in. Enjoy the celebration!' : 'Your arrival has been recorded. Enjoy the celebration!'}
        </p>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#FAF6EF', display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem' }}>
      <div style={{ textAlign:'center', maxWidth:'340px', width:'100%' }}>
        ...
        {functionName && (
          <p style={{ fontFamily:'Georgia, serif', color:'#9A2143', fontSize:'1rem', marginBottom:'1.5rem' }}>{functionName}</p>
        )}
        <div style={{ background:'#fff', borderRadius:'16px', border:'1px solid #E8D5A3', padding:'2rem 1.5rem', marginBottom:'1.5rem' }}>
          <div style={{ width:'64px', height:'64px', borderRadius:'50%', background:'#9A2143', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 1rem', fontFamily:'Georgia, serif', fontSize:'1.6rem', color:'#FAF6EF' }}>
            {guest?.full_name?.[0] || '?'}
          </div>
          <h1 style={{ fontFamily:'Georgia, serif', fontSize:'1.6rem', color:'#1B2B4B', fontWeight:400, marginBottom:'0.25rem' }}>{guest?.full_name}</h1>
          {guest?.group_tag && (
            <p style={{ color:'#BFA054', fontSize:'0.8rem', letterSpacing:'0.08em', marginTop:'0.25rem' }}>{guest.group_tag}</p>
          )}
        </div>
        <button onClick={handleCheckIn} disabled={checking}
          style={{ width:'100%', padding:'1.1rem', background: checking ? '#aaa' : '#9A2143', color:'#FAF6EF', border:'none', borderRadius:'12px', fontFamily:'Georgia, serif', fontSize:'1.1rem', cursor: checking ? 'not-allowed' : 'pointer', letterSpacing:'0.05em', transition:'background 0.2s' }}>
          {checking ? 'Checking in...' : "I've Arrived ✓"}
        </button>
        <p style={{ color:'#aaa', fontSize:'0.72rem', marginTop:'1rem', lineHeight:1.6 }}>Tap the button above to confirm your arrival</p>
      </div>
    </div>
  )
}